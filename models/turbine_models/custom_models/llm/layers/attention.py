...
__all__ = ["enable_custom_attention"]
...
def llama_pos_shift_attention_forward(
    ...
    if self.num_key_value_groups == 1:
        ...
        attn_output = ops.bmm_bcast_rhs(attn_weights, value_states).flatten(1, 2)
...
MODE_TO_FN = {
    "streaming_llm": llama_pos_shift_attention_forward,
    "default": shark_efficient_attention,
}


def enable_custom_attention(model, mode="default"):
    if isinstance(model, LlamaAttention):
        model.forward = types.MethodType(MODE_TO_FN[mode], model)
    for name, module in reversed(model._modules.items()):
        ...
        if isinstance(module, LlamaAttention):
            model._modules[name].forward = types.MethodType(
                MODE_TO_FN[mode], model._modules[name]
            )
